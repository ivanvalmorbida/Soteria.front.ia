<template>
  <LayoutApp>
    <div class="max-w-2xl mx-auto animate-fade-in">
      <div class="mb-8">
        <router-link to="/historicos-contabeis" class="text-primary-600 hover:text-primary-700 flex items-center mb-4">
        ...
        <h1 class="text-3xl font-bold text-dark-100">
          {{ isEditing ? 'Editar Histórico Contábil' : 'Novo Histórico Contábil' }}
        </h1>
      </div>
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="flex justify-end gap-3">
          <router-link to="/historicos-contabeis" class="btn-secondary">Cancelar</router-link>
...
const loadItem = async () => {
  ...
  try {
    loading.value = true
    const data = await historicoContabilService.getById(route.params.id)
    form.value = { ...data }
  } catch (error) {
    toast.error('Erro ao carregar dados')
    router.push('/historicos-contabeis')
  } finally {
    loading.value = false
  }
}
...
const handleSubmit = async () => {
  try {
    saving.value = true
    if (isEditing.value) {
      await historicoContabilService.update(form.value.codigo, { descricao: form.value.descricao })
      toast.success('Histórico atualizado com sucesso')
    } else {
      await historicoContabilService.create({ descricao: form.value.descricao })
