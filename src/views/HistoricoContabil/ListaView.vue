  ...
  <LayoutApp>
    <div class="animate-fade-in">
      <div class="flex justify-between items-center mb-8">
        <div>
          <h1 class="text-3xl font-bold text-dark-100 mb-2">Históricos Contábeis</h1>
          <p class="text-dark-300">Gerencie os históricos contábeis padrão</p>
        </div>
        <router-link to="/historicos-contabeis/novo" class="btn-primary">
              ...
              <tr v-for="item in itens" :key="item.codigo" class="hover:bg-dark-800 transition-colors">
                ...
                <td class="table-cell text-right">
                  <div class="flex justify-end gap-2">
                    <router-link
                      :to="`/historicos-contabeis/${item.codigo}/editar`"
      ...
      <div v-else class="card p-16 text-center">
        <h3 class="text-xl font-semibold text-dark-100 mb-2">Nenhum histórico encontrado</h3>
        <p class="text-dark-300 mb-6">Comece cadastrando um novo histórico contábil</p>
        <router-link to="/historicos-contabeis/novo" class="btn-primary inline-flex items-center">
...
const loadItens = async () => {
  try {
    loading.value = true
    itens.value = await historicoContabilService.getAll()
  } catch (error) {
...
const handleSearch = async () => {
  ...
  try {
    loading.value = true
    itens.value = await historicoContabilService.buscarPorDescricao(searchTerm.value)
  } catch (error) {
...
const handleDelete = async () => {
  try {
    deleting.value = true
    await historicoContabilService.delete(itemToDelete.value.codigo)
