    ...
    <div class="max-w-4xl mx-auto animate-fade-in">
      <div class="mb-8">
        ...
        <div class="flex justify-between items-start">
          <div>
            <h1 class="text-3xl font-bold text-dark-100 mb-2">{{ pessoa?.nome }}</h1>
            <p class="text-dark-300">Código: #{{ pessoa?.codigo }}</p>
          </div>
          <router-link :to="`/pessoas-fisicas/${pessoa?.codigo}/editar`" class="btn-primary">
      ...
      <div v-else-if="pessoa" class="space-y-6">
        ...
        <div class="card p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="text-sm font-medium text-dark-300">CPF</label>
              <p class="text-lg font-mono">{{ formatCpf(pessoa.cpf) }}</p>
            </div>
            <div>
              <label class="text-sm font-medium text-dark-300">Data de Nascimento</label>
              <p class="text-lg">{{ formatDate(pessoa.nascimento) }}</p>
            </div>
            <div>
              <label class="text-sm font-medium text-dark-300">Sexo</label>
              <p class="text-lg">{{ formatSexo(pessoa.sexo) }}</p>
            </div>
            <div>
              <label class="text-sm font-medium text-dark-300">Cadastrado em</label>
              <p class="text-lg">{{ formatDate(pessoa.cadastro) }} {{ formatTime(pessoa.cadastro) }}</p>
            </div>
          </div>
        </div>
...
const loadPessoa = async () => {
  try {
    loading.value = true
    pessoa.value = await pessoaFisicaService.getById(route.params.id)
  } catch (error) {
